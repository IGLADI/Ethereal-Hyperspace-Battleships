...
class EventCommands(commands.Cog):
    ...
    async def event(self, interaction: discord.Interaction):
        if data.event_manager is None or data.event_manager.events == {}:
            banner = ErrorBanner(
                text="There is no event running right now.", user=interaction.user
            )
        ...
        banner = SuccessBanner(
            text=f"{player.name} joined the event!\nParticipants: {participants_text}",
            user=interaction.user,
        )
        await interaction.response.send_message(embed=banner.embed)
        private_banner = SuccessBanner(
            text="Try to find Ruebñ's lost ship by scanning the search area, if you found it, use /locate [x_pos] [y_pos] to let Ruebñ know!\n",
            user=interaction.user,
        )
    ...
    async def locate(self, interaction: discord.Interaction, x_pos: int, y_pos: int):
        if data.event_manager is None or data.event_manager.events == {}:
            banner = ErrorBanner(
                text="There is nothing lost right now. So no need to locate somthing.",
                user=interaction.user,
            )
            await interaction.response.send_message(embed=banner.embed, ephemeral=True)
            return
        ...
        if player not in data.event_manager.events[1].participants:
            banner = ErrorBanner(
                text="You are not participating in this event. Use /join_event to help searching.",
                user=interaction.user,
            )
            await interaction.response.send_message(embed=banner.embed, ephemeral=True)
            return
        ...
        if (
            data.event_manager.events[1].x_pos == x_pos
            and data.event_manager.events[1].y_pos == y_pos
        ):
            for participant in data.event_manager.events[1].participants:
                participant.ship.modules["Cargo"].add_resource(
                    data.event_manager.events[1].prize[0],
                    data.event_manager.events[1].prize[1],
                )
            banner = SuccessBanner(
                text=f"Congratulations, you found the lost ship!\nAll participants got {data.event_manager.events[1].prize[1]} tons of {data.event_manager.events[1].prize[0]}!",
                user=interaction.user,
            )
            await interaction.response.send_message(embed=banner.embed)
        ...
        banner = ErrorBanner(
            text="Sorry, you didn't find the lost ship. Try again!",
            user=interaction.user,
        )
        await interaction.response.send_message(embed=banner.embed, ephemeral=True)
        return
